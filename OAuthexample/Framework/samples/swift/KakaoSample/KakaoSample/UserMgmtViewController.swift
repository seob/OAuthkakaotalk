            ...
            if let error = error as NSError? {
                ...
                if displayResult {
                    ...
                    if let account = me.account {
                        ...
                        } else if account.hasPhoneNumber == .true {
                            message.append("있음 (사용자 동의가 필요함)")
                        } else if account.hasPhoneNumber == .false {
                            message.append("없음")
                        } else if account.hasPhoneNumber == .null {
                            message.append("없음 (앱 권한이 필요함)")
                        }
                        ...
                        } else if account.hasBirthday == .true {
                            message.append("있음 (사용자 동의가 필요함)")
                        } else {
                            message.append("없음")
                        }
                    ...
                    if let accout = me.account {
                        if accout.needsScopeAccountEmail() {
                        ...
                        if accout.needsScopePhoneNumber() {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if self.doneSignup == false {
            ...
            case 0:
                requestMe(true)
            case 1:
                KOSessionTask.profileUpdate(withProperties: getFormData() as! [String : String], completionHandler: { (success, error) -> Void in
                    if let error = error as NSError? {
                        UIAlertController.showMessage(error.description)
                    } else {
