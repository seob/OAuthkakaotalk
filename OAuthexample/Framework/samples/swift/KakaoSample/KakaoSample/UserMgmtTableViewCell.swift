...
class UserMgmtTableViewCell: UITableViewCell, UITextFieldDelegate {
    ...
    func setMe(me: KOUserMe?) -> Void {
        ...
        if let me = me {
            ...
            } else if me.account?.hasEmail == .true {
            ...
            } else if me.account?.hasPhoneNumber == .true {
                self.phoneNumber.text = "전화번호 있음 (사용자 동의가 필요함)"
            } else if me.account?.hasPhoneNumber == .false {
                self.phoneNumber.text = "전화번호 없음"
            } else if me.account?.hasPhoneNumber == .null {
                self.phoneNumber.text = "전화번호 없음 (권한이 필요함)"
            }
    ...
    func getProperties() -> [AnyHashable: Any] {
